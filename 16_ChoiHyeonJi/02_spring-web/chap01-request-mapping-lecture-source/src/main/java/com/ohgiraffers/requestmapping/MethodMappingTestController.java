...
* 다양한 @RequestMapping 설정 내용을 따ㅡㄹㄴ다
*
...
public class MethodMappingTestController {
  ...
  public String menuRegist(Model model) {
    ...
    /* 핸들러 메서드의 return 값은
    * 반환하고자 하는 view(html 파일)
    * 경로를 포함한 이름을 작성한다.
    * resources/templates 폴더 하위부터 작성.
    * .html은 생략하여 작성
    * -> 자세한건 View Resolver 공부할 때 함.
    * */
  ...
  public String getMenuModify(Model model) { //MVC 모델이 아님

    // Model 객체 : 화면에 데이터를 전달하는 용도의 객체(View에게 넘겨주는 데이터)
    model.addAttribute("message", "GET 방식 메뉴 수정");

    // forward 구문
    // - resources/template/ 폴더 기준으로 파일 경로 작성
    // - 마지막 확장자 .html 생략
    return "mappingResult"; // 이렇게 클라이언트에게 응답하겠다 // 응답할 때 message로 보냄
  }
